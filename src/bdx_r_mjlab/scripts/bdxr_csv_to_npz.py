"""Convert a BDX-R retargeted motion CSV to the NPZ format expected by mjlab tracking.
...
Usage:
  MUJOCO_GL=egl uv run -m bdx_r_mjlab.scripts.bdxr_csv_to_npz \\
...
from bdx_r_mjlab.tasks.tracking.env_cfg import bdxr_flat_tracking_env_cfg
    ...
    while not file_saved:
        ...
        if not file_saved:
            ...
            if reset_flag and not file_saved:
                ...
                run = wandb.init(project="bdxr_csv_to_npz", name=COLLECTION)
...
def main(
    ...
    scene = Scene(bdxr_flat_tracking_env_cfg().scene, device=device)
    model = scene.compile()

"""Convert a BDX-R full body motion CSV to the NPZ format expected by mjlab tracking.
...
Usage:
  MUJOCO_GL=egl uv run -m bdx_r_mjlab.scripts.bdxr_full_csv_to_npz \\
...
from bdx_r_mjlab.tasks.tracking.full_env_cfg import bdxr_full_flat_tracking_env_cfg
...
# Joint order must match columns 7+ of your CSV file.
BDXR_JOINT_NAMES = [
    "Left_Hip_Yaw",
    "Left_Hip_Roll",
    "Left_Hip_Pitch",
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        # CSV is xyzw, convert to wxyz for mjlab internals.
    ...
    while not file_saved:
        ...
        if not file_saved:
            ...
            if reset_flag and not file_saved:
                ...
                local_npz = Path(f"./data/{output_name}.npz")
                local_npz.parent.mkdir(parents=True, exist_ok=True)

                print(f"Saving to {local_npz} ...")
                np.savez(str(local_npz), **log)
                print(f"[INFO]: Motion saved locally → {local_npz.resolve()}")
                print(f"[INFO]: To train without WandB, pass:")
                print(f"        --env.commands.motion.motion-file {local_npz.resolve()}")
                ...
                run = wandb.init(project="bdxr_full_csv_to_npz", name=COLLECTION)
                print(f"[INFO]: Uploading motion to wandb: {COLLECTION}")
                REGISTRY = "motions"
                logged_artifact = run.log_artifact(
                    artifact_or_path=str(local_npz), name=COLLECTION, type=REGISTRY
                )
                try:
                    run.link_artifact(
                    ...
                    print(f"[INFO]: Motion linked to wandb registry: {REGISTRY}/{COLLECTION}")
                except NotImplementedError:
                    print(
                        "[WARN]: WandB offline mode — registry link skipped. "
                        "Run `wandb sync` later to push, or use the local file above."
                    )
...
def main(
    ...
    """Replay BDX-R motion from CSV and save forward-kinematics data to NPZ.

    Args:
        input_file: Path to the input CSV file.
        output_name: Name for the output artifact in WandB.
        input_fps: Frame rate of the CSV file.
        output_fps: Desired output frame rate (should match sim timestep * decimation).
    ...
    scene = Scene(bdxr_full_flat_tracking_env_cfg().scene, device=device)
    model = scene.compile()

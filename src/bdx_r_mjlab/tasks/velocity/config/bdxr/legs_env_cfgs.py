...
from bdx_r_mjlab.robots import (
  BDXR_ACTION_SCALE_LEGS,
  get_bdxr_robot_legs_cfg,
)
...
def bdxr_rough_env_legs_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  cfg.scene.entities = {"robot": get_bdxr_robot_legs_cfg()}
  ...
  joint_pos_action.scale = BDXR_ACTION_SCALE_LEGS
  ...
  cfg.rewards["pose"].params["std_walking"] = {
    r".*_Hip_Pitch.*": 0.5, r".*_Hip_Roll.*": 0.06, r".*_Hip_Yaw.*": 0.15,
    r".*_Knee.*": 0.5, r".*_Ankle.*": 0.1,
  }
  cfg.rewards["pose"].params["std_running"] = {
    r".*_Hip_Pitch.*": 0.8, r".*_Hip_Roll.*": 0.06, r".*_Hip_Yaw.*": 0.2,
